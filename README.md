...
**重要说明 (Notes):**
* 在两台服务器上进行了docker部署，且均能正常运行

**下一步计划（Todo）:**
* 完成sdc-net docker的存档与后续使用
...
**重要说明 (Notes):**
* 修改了封装的SDC-Net项目的内部路径，优化了外部路径的轻量化挂载

**下一步计划（Todo）:**
* 进行跨设备测试实验，确保docker在不同设备上都能正常运行。
...
容器启动后，需要通过以下命令进入容器内部的终端：

```bash
# 1. 查看正在运行的容器 ID
docker ps

# 2. 进入容器终端 (将 <container_id> 替换为实际ID)
docker exec -it <container_id> /bin/bash
提示：进入成功后，终端提示符通常会变为 root@<container_id>:/app# 或类似格式。
```
